...
const ActiveAlerts: React.FC = () => {
  ...
  const { addNotification, incrementAlertCount, setAlertCount } = useNotifications();
  ...
  // Update alert count in sidebar whenever alerts change
  useEffect(() => {
    const activeCount = alerts.filter(a => a.status === 'Open').length;
    setAlertCount(activeCount);
  }, [alerts, setAlertCount]);

  // Toast trigger with notification
  const triggerAlertToast = useCallback((message: string, alertData?: { type: string; location: string; severity: string }) => {
    setToastMessage(message);
    setTimeout(() => {
      setToastMessage(prev => (prev === message ? null : prev));
    }, 5000);
    
    // Also add to notification panel if alert data provided
    if (alertData) {
      addNotification({
        title: `New Alert: ${alertData.type}`,
        message: message,
        type: 'alert',
    ...
    const fetchBridgeData = async () => {
      try {
        ...
        if (data.isNew && lastAlertIdRef.current !== latestId) {
          ...
          // Show toast notification with alert data for notification panel
          triggerAlertToast(
            `🚨 New Citizen Report: ${data.type} at ${data.location}`,
            {
...
#DelhiFightsPollution #EmergencyResponse`;
  ...
  const handleBroadcastToTwitter = async () => {
    try {
      ...
      // Also save to localStorage for Social Media Control panel
      if (selectedAlert) {
        savePost({
          content: broadcastMessage,
          platforms: ['twitter'],
          reason: `Action Plan: ${selectedAlert.title}`,
          location: selectedAlert.location,
